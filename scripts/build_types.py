...
# https://github.com/aproposmath/stationeers-pytrapic/blob/main/scripts/build_types.py

# pyright: basic, reportArgumentType=false, reportOperatorIssue=false, reportOptionalMemberAccess=false, reportOptionalSubscript=false
...
field_types: dict[str, str] = {
    ...
    "Color": "_Color",
    #
    **{(name + suf): "float" for name in gases for suf in ["", "Input", "Output", "Output2"]},
}

_run_date = datetime.now().strftime("%Y-%m-%d")

__header = f"""
# This file is generated by {Path(__file__).name} on {_run_date}.
# do NOT modifiy
...
__generic_structures_property = """
    {property_name}: _FieldDesc = _mk_field()
"""
...
class Property:
    ...
    def generate_code(self, multiple: bool = False) -> str:
        ...
        if self.can_write:
            if typ := field_types.get(name):
                code += f"  {name}: _FieldDescW[{typ}] = _mk_field({typ})\n"
            else:
                code += f"  {name}: _FieldDescW = _mk_field()\n"
        elif self.can_read:
            if typ := field_types.get(name):
                code += f"  {name}: _FieldDesc[{typ}] = _mk_field_ro({typ})\n"
            else:
                code += f"  {name}: _FieldDesc = _mk_field_ro()\n"
        return code
...
def generate_generic_structure(struct: Structure, multiple: bool = False) -> str:
    ...
    bases = (
        ", ".join([f"_DeviceTyped" + suffix] + [b + suffix for b in sorted(struct.bases)])
        if not is_base
        else ""
    )
